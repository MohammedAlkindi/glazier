...
func defaultCoInitializeEx(pvReserved uintptr, dwCoInit uint32) error {
	r1, _, _ := syscall.SyscallN(procCoInitializeEx.Addr(), pvReserved, uintptr(dwCoInit))
	if int32(r1) < 0 {
...
func defaultMFStartup(version uint32, flags uint32) error {
	r1, _, _ := syscall.SyscallN(procMFStartup.Addr(), uintptr(version), uintptr(flags))
	if int32(r1) < 0 {
...
func defaultCOMSetGUID(c *comObject, key *windows.GUID, val *windows.GUID) error {
	if c == nil || c.vtbl == nil {
